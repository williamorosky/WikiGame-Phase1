...
@objc public protocol GameTextViewDelegate: UITextViewDelegate {
    @objc optional func textViewDidChangeHeight(_ textView: GameTextView, height: CGFloat)
}

@IBDesignable
open class GameTextView: UITextView {
    ...
    @IBInspectable open var maxLines: Int = 0 {
        didSet { forceLayoutSubviews() }
    }
    ...
    @IBInspectable open var underLineWidth: CGFloat = 2.0 {
        didSet { forceLayoutSubviews() }
    }
    @IBInspectable open var underLineColor: UIColor = .black {
        didSet { forceLayoutSubviews() }
    }
    @IBInspectable open var underLineAlphaBefore: CGFloat = 0.2 {
        didSet { forceLayoutSubviews() }
    }
    @IBInspectable open var underLineAlphaAfter: CGFloat = 1 {
        didSet { forceLayoutSubviews() }
    }
    ...
    override open func layoutSubviews() {
        ...
        if height != heightConstraint!.constant {
            ...
            if let delegate = delegate as? GameTextViewDelegate {
    ...
    private func scrollToCorrectPosition() {
        if self.isFirstResponder {
            ...
            isScrollEnabled = self.numberOfVisibleLines < self.maxLines
    ...
    func drawLine() {
        ...
        underLine.alpha = text.count == 0 ? underLineAlphaBefore : underLineAlphaAfter
    ...
    @objc func didBeginChangeText(notification: Notification) {
        if let sender = notification.object as? GameTextView, sender == self {
