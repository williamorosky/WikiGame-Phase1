...
open class PressableButton: UIButton {
    ...
    @IBInspectable
    public var offset: Double = Defaults.offset {
        didSet {
            updateBackgroundImages()
            updateTitleInsets()
            updateImageInsets()
    ...
    func updateTitleInsets() {
        let topPadding = isHighlighted ? shadowHeight * CGFloat(offset) : 0
        let bottomPadding = isHighlighted ? shadowHeight * (1 - CGFloat(offset)) : shadowHeight
